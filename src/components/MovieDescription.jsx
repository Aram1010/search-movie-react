...
const MovieDescription = ({ movie }) => {
  return (
    <div>
      <h2
        style={{ fontSize: "clamp(10px , 8vw , 60px)" }}
        className="text-700 max-w-[350px] mx-[20px] mt-[30px]"
      >
        {movie ? movie.title : null}
      </h2>
      <div
        className="absolute bottom-0 smax:left-[50%] smax:translate-x-[-50%] 
                  w-[90%] max-w-[800px] h-[40%] min-h-[300px] bg-[rgba(236,240,243,.2)] 
      ...
      >
        <div className="grid grid-cols-2 grid-rows-2 gap-[30px] h-[90%] absolute">
          ...
          <span>
            ...
            <p className="max-h-[100px] max-w-[300px] text-[15px] smin:text-[16px] overflow-y-scroll scrollbar-hide">
              {movie.overview}
            </p>
          </span>
          <span>
          ...
          <span>
            <button
              ...
              className="flex items-center bg-[#ffff] py-[12px] px-[18px] smin:text-[18px] smin:px-[20px] smin:py-[12px] 
                  ...
                  rounded-[50px] absolute bottom-[1rem] right-[1rem] 
